...
void BacaTIME (TIME * T){
    ...
    while (!IsTIMEValid(hour, min, sec)){
        printf("Jam tidak valid\n");
        scanf("%d %d %d", &hour, &min, &sec);
    }
    CreateTime(T, hour, min, sec);
}
...
TIME DetikToTIME (long N){
    ...
    Hour(detik_to_time) = N/3600;
    Minute(detik_to_time)= (N%3600)/60;
    Second(detik_to_time) = (N%3600)%60;
    return(detik_to_time);
} 
...
long Durasi (TIME TAw, TIME TAkh){
    long duration;
    duration= TIMEToDetik(TAkh)-TIMEToDetik(TAw);
    if(duration <0){
